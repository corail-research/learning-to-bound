 ...
 */
void MinLongestPath::merge_layer(int layer, vector<Node2*> &nodes_layer) {

	sort(nodes_layer.begin(), nodes_layer.end(), CompareNode2sLongestPath());
	...
	for( vector<Node2*>::iterator node = nodes_layer.begin()+width; node != nodes_layer.end(); ++node) {
/*
        cout << "Node2s to merges" << endl;
        cout << "n1: " << *state << endl;
 ...
 */
void PairMinLongestPath::merge_layer(int layer, vector<Node2*> &nodes_layer) {
	...
	for( vector<Node2*>::iterator node = nodes_layer.begin(); node != nodes_layer.end(); node++ ) {
	...
	while( current_size > width ) {
		...
		// remove last node from BDD2 (without consistency check)
		delete nodes_layer[current_size-1];
		nodes_layer.pop_back();
		current_size--;

		// now, we must check if the state of the new node appears in any previous node
		Node2Map2::iterator map_it = current_states.find(&(nodes_layer[current_size-1]->state));
 ...
 */
void ConsecutivePairLongestPath::merge_layer(int layer, vector<Node2*> &nodes_layer) {
...
//		cout << "new_nodes: " << nodes_layer.size() << endl;
		...
		sort(old_nodes.begin(), old_nodes.end(), CompareNode2sLongestPath());
 ...
 */
void LexicographicMerger::merge_layer(int layer, vector<Node2*> &nodes_layer) {
	...
	Node2* last, *previous_to_last;

	while( (int)current_states.size() > width ) {

		// take last two nodes of list
		Node2Map2::reverse_iterator rev_map_it = current_states.rbegin();
		...
		// now, we must check if the state of the new node appears in any previous node
		Node2Map2::iterator map_it = current_states.find(&(previous_to_last->state));
		if( map_it != current_states.end() ) {
	...
	nodes_layer.clear();
	for( Node2Map2::iterator node = current_states.begin(); node != current_states.end(); node++ ) {
 ...
 * Min Size Merger
 */
void MinSizeMerger::merge_layer(int layer, vector<Node2*> &nodes_layer) {
	...
	node_list.sort(CompareNode2sStateSizeAscending());

	// auxiliaries
	list<Node2*> merger_list;
	Node2* last, *previous_to_last;
	bool found;
	...
	while( (int)node_list.size() > width ) {
		...
		if( found ) {
			...
			node_list.merge(merger_list, CompareNode2sStateSizeAscending());
		}
	}

	// replace nodes of vector from nodes in the list
	nodes_layer.clear();
	for( list<Node2*>::iterator node = node_list.begin(); node != node_list.end(); node++ ) {
 ...
 * Max Size Merger
 */
void MaxSizeMerger::merge_layer(int layer, vector<Node2*> &nodes_layer) {
	...
	// sort list according to state size
	node_list.sort(CompareNode2sStateSizeDescending());
	...
	while( (int)node_list.size() > width ) {
		...
		if( found ) {
			...
			merger_list.clear();
			merger_list.push_back(previous_to_last);
			node_list.merge(merger_list, CompareNode2sStateSizeDescending());
		}
	}
 ...
 */
void SymmetricDifferenceMerger::merge_layer(int layer, vector<Node2*> &nodes_layer) {
	...
	ComparatorNode2PairSymmLP pairs_comp(symm_diff_vals, longest_path_val);
	Node2* last, *previous_to_last;
	...
	while( (int)current_states.size() > width ) {
		...
		for( Node2Map2::iterator nodeA = current_states.begin(); nodeA != current_states.end(); nodeA++ ) {
			Node2Map2::iterator nodeB = nodeA;
			nodeB++;
...
//				cout << " --> " << aux.set << " - size: " << aux.set.count() << endl;
				...
				node_pairs.push_back(pair<Node2*,Node2*>(nodeA->second, nodeB->second));
			}
		}
 ...
 * Random merger
 */
void RandomMerger::merge_layer(int layer, vector<Node2*> &nodes_layer) {
