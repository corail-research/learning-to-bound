...
void IndepSetSolver::initialize(IntSet &initial_state, int initial_longest_path) {
	...
	initial_node = new Node2(initial_state, initial_longest_path);
	node_list.clear();
...
int IndepSetSolver::generate_next_step_restriction(int next_vertex) {
    ...
    if( width != EXACT_BDD2 && (int)nodes_layer.size() > width ) {
     ...
     */
    Node2* branch_node;
    for( vector<Node2*>::iterator it = nodes_layer.begin(); it != nodes_layer.end(); ++it ) {
        ...
        node = new Node2(branch_node->state, branch_node->longest_path+inst->weights[current_vertex]);
...
int IndepSetSolver::generate_relaxation(IntSet &initial_state, int initial_longest_path) {
	...
	//Node2* initial_node = new Node2(initial_state, initial_longest_path, true); // b&b
	Node2* initial_node = new Node2(initial_state, initial_longest_path);
	node_list.clear();
	node_list[&(initial_state)] = initial_node;

	Node2Map2::iterator node_it, existing_node_it;
	Node2* node;
...
//		for( Node2Map2::iterator node_it = node_list.begin();
...
//		cout << endl;
		...
		for( vector<Node2*>::iterator it = nodes_layer.begin(); it != nodes_layer.end(); ++it ) {
			...
			// add node to final BDD2 representation (bdd-save)
			...
			// **** one arc ****
			//node = new Node2(branch_node->state, branch_node->longest_path+1, branch_node->exact);
...
//				existing_node_it->second->one_ancestors.push_back(branch_node);
			...
			if( existing_node_it != node_list.end() ) {
				...
				// update node links (bdd-save)
				//node = new Node2(branch_node->state, branch_node->longest_path);
				//node_list[&(node->state)] = node;
...
//		int id = 0;
//		for( vector<Node2*>::iterator it = final_bdd[i].begin(); it != final_bdd[i].end(); it++ ) {
//			assert( (*it)->layer != -1 );
//			(*it)->id = id++;
//		}
...
//		cout << endl;
		...
		for( vector<Node2*>::iterator it = nodes_layer.begin(); it != nodes_layer.end(); ++it ) {
			...
			// **** one arc ****
			//node = new Node2(branch_node->state, branch_node->longest_path+1, branch_node->exact); (b&b)
			node = new Node2(branch_node->state, branch_node->longest_path+1);
			for( vector<int>::iterator v = inst->graph->adj_list[current_vertex].begin();
					v != inst->graph->adj_list[current_vertex].end();
					v++ )
			{
