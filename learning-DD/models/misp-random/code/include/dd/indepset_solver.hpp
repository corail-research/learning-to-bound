...
/**
 * Node2 comparator by relax ub
 */
struct CompareNode2sRelaxUB {
...
typedef priority_queue<Node2*, vector<Node2*>, CompareNode2sRelaxUB> Node2Queue;
...
struct IndepSetSolver {
	...
	vector<Node2*>					nodes_layer;			      /**< nodes in a layer */

	vector<Node2*>					temporary_branch_nodes;
	...
	IS_Ordering2*					ordering;			         /**< ordering */
	...
	int eligible_vertex;
	Node2* initial_node;
	Node2Map2::iterator node_it;
	Node2Map2::iterator existing_node_it;
	int current_vertex;
	Node2* node;
	...
	void update_node_match(Node2* nodeA, Node2* nodeB);
	...
	IndepSetSolver(IndepSetInst2* _inst, int _width);
};


inline IndepSetSolver::IndepSetSolver(IndepSetInst2* _inst, int _width) : inst(_inst), width(_width)
{
	relax = false;
...
inline void IndepSetSolver::update_node_match(Node2* nodeA, Node2* nodeB) {
...
inline void add_without_repetition(vector<Node2*> &v, Node2* node) {
	for( vector<Node2*>::iterator it = v.begin(); it != v.end(); it++ ) {
		if( (*it) == node )
			return;
	}
	v.push_back(node);
...
inline int IndepSetSolver::choose_next_vertex_min_size_next_layer_random() {
	...
	RandomizedMinInState22* min_state = (RandomizedMinInState22*) ordering;
