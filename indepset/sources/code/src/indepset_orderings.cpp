...
int MinInState::vertex_in_layer(int layer) {
	return -1;
}

// fixed ordering
void FixedOrdering::set_ordering(int* sol, int act_size) {
	
	v_in_layer.resize((act_size+inst->graph->n_vertices)*10);
	for( int v = 0; v < act_size; v++ ) {
		v_in_layer[v] = sol[v];
		cout << "Read ordering " << v << endl;
	}
}
...
void RandOrdering::construct_ordering() {
	for( int i = 0; i < inst->graph->n_vertices; i++ ) {
		v_in_layer[i] = i;
	}
